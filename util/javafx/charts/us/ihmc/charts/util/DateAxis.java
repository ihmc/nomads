...
/**
 * An axis that displays time values
...
public class DateAxis extends Axis<Date>
{
    ...
    private Interval actualInterval = Interval.hour_1;
    ...
    protected List<Date> calculateTickValues (double length, Object range)
    {
        ...
        double averageTicks = length / averageTickGap;
        ...
        // If there are at least three dates, check if the gap between the lower date and the second date is at least
        // half the gap of the second and third date.
...
//        }
    ...
    private List<Date> makeDatesEven (List<Date> dates, Calendar calendar)
    {
        ...
        if (dates.size() > 2) {
            ...
            // This happens if you display days and then add a date many years in the future the tick label will still
            // be displayed as day.
            for (int i = 0; i < dates.size(); i++) {
                calendar.setTime (dates.get(i));
                switch (actualInterval.interval) {
                    case Calendar.YEAR:
                        // If its not the first or last date (lower and upper bound), make the year begin with first
                        // month and let the months begin with first day.
     ...
     * calculate the tick values for this interval.
