...
/**
 * Controller class for the Line 2D chart where the x axis has numeric values
 * @author Rita Lenzi (rlenzi@ihmc.us)
 */
public class Line2DXNumberController
{
    /**
    ...
    public void setData (String title, String xLabel, String yLabel, XYData data)
    {
        ...
        for (XYValue xyValue : data.getValues()) {
            series.getData().add (new XYChart.Data<>((Number) xyValue.x, (Number) xyValue.y));
        }

        _line2DChart.getData().add (series);
    }

    @FXML private LineChart<Number, Number> _line2DChart;
