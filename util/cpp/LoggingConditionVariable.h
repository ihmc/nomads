...
#ifndef INCL_LOGGING_CONDITION_VARIABLE_H
#define INCL_LOGGING_CONDITION_VARIABLE_H
...
namespace NOMADSUtil
{
    ...
    class LoggingConditionVariable
    {
        public:
            LoggingConditionVariable (LoggingMutex *pMutex);
            ~LoggingConditionVariable();
            ...
            int wait (uint16 ui16LocationId);
            int wait (uint16 ui16LocationId, int64 i64Millisec);
            int wait (uint16 ui16LocationId, int64 i64Millisec, bool *timedOut);
            ...
            int notify (uint16 ui16LocationId);
            int notifyAll (uint16 ui16LocationId);

        protected:
            LoggingMutex *_pMutex;
            #if defined (WIN32)
                HANDLE _hEventObj;
