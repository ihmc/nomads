...
NetworkMessageReceiver::NetworkMessageReceiver (NetworkMessageService *pNMSParent, NetworkInterface *pNetIface,
                                                bool bReceive)
...
int NetworkMessageReceiver::receive (char *achBuf, int iBufLen)
{
    ...
    if (rc < 0) {
        ...
        checkAndLogMsg (pszMethodName, Logger::L_HighDetailDebug, "Socket timed out.\n");
    }
    else if (remoteAddr.getIPAddress() != incomingIfaceByAddr.getIPAddress()) {
        ...
        if ((pNetMsg != NULL) && (!pNetMsg->isMalformed())) {
            ...
            checkAndLogMsg (pszMethodName, Logger::L_MediumDetailDebug,
                            "message src:%u - lasthop: %u - msg size: %u \n",\
                            pNetMsg->getSourceAddress(), remoteAddr.getIPAddress(),
                            pNetMsg->getMsgLen());

            if (!_pNMSParent->isLocalAddress (remoteAddr.getIPAddress()) &&
                !_pNMSParent->isLocalAddress (pNetMsg->getSourceAddress())) {
                if (_bEstimateRate && _pRateEstimator != NULL) {
                    _pRateEstimator->update (remoteAddr.getIPAddress(), (uint32) rc);
                }
                pNetMsg->incrementHopCount();
                InetAddr dst (pNetMsg->getTargetAddress());
                _pNMSParent->messageArrived (pNetMsg, incomingIfaceByAddr.getIPAsString(),
                                             remoteAddr.getIPAddress());
            }
            else {
