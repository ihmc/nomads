...
uint32 NetworkMessage::getSourceAddress (void) const
{
    return *((uint32*)(_pBuf + SOURCE_ADDRESS_FIELD_OFFSET));
}

uint32 NetworkMessage::getTargetAddress (void) const
{
...
{
    const char * const pszMethod = "NetworkMessageInterface::display";
    checkAndLogMsg (pszMethod, Logger::L_HighDetailDebug, "Message %u:%u\n", getSourceAddress(), getMsgId());
...
void NetworkMessage::dump (FILE *file)
{
    ...
    fprintf(file, "Source add: %u\t", getSourceAddress ());
    fprintf(file, "Target add: %u\t", getTargetAddress ());
