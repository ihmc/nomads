...
DTLSCommInterface::DTLSCommInterface (CommInterface * pCI, bool isServer, DtlsEndpoint ** dtls)
{
    ...
    _mtu                            = DEFAULT_MTU;
    _pCI                            = pCI;
    _pcLastHanshakeMessage          = nullptr;
    _pLastRemoteAddr                = nullptr;
    _lastMessageSize                = 0;
...
DTLSCommInterface::DTLSCommInterface (CommInterface * pCI, bool bIsServer, uint32 ui32RcvTimeout,
                                      const char * pszPathToCertificate, const char * pszPathToPrivateKey)
{
    const char * const pszMethodName = "DTLSCommInterface::DTLSCommInterface";
    checkAndLogMsg (pszMethodName, Logger::L_HighDetailDebug,
                    "Recreating the DTLS object\n");

    _bIsServer = bIsServer;
    setCertificatePaths(pszPathToCertificate, pszPathToPrivateKey);
    if ((_pDTLS = new DtlsEndpoint(_sPathToCertificate.c_str(), _sPathToPrivateKey.c_str())) == nullptr) {
...
DTLSCommInterface::~DTLSCommInterface (void)
{
    ...
    if (_pcLastHanshakeMessage != nullptr) {
        delete _pcLastHanshakeMessage;
        _pcLastHanshakeMessage = nullptr;
    }

    if (_pLastRemoteAddr != nullptr) {
        delete _pLastRemoteAddr;
        _pLastRemoteAddr = nullptr;
    }
}

bool DTLSCommInterface::freeLastHanshakeMessage (void)
{
    const char * const pszMethodName = "DTLSCommInterface::freeLastHanshakeMessage";
...
{
    const char * const pszMethodName = "DTLSCommInterface::handleInitializationError";
    if (errorCode != 0) {
        if (isServer) {
...
{
    const char * const pszMethodName = "DTLSCommInterface::handleNoRcv";
    checkAndLogMsg (pszMethodName, Logger::L_HighDetailDebug,
                    "Rcv timeout (%d), trying again...\n", _ui32RcvTimeout);
...
{
    const char * const pszMethodName = "DTLSCommInterface::handleRcvError";
    checkAndLogMsg (pszMethodName, Logger::L_Warning,
                    "Rcv failed (rc = %d), trying again...\n", rc);
}

int DTLSCommInterface::handleRcv (const int bufSize, char * pBuf, const int handshakeStep, InetAddr * pRemoteAddr)
{
    const char * const pszMethodName = "DTLSCommInterface::handleRcv";
    checkAndLogMsg (pszMethodName, Logger::L_HighDetailDebug,
                    "Received <%dB>\n", bufSize);

    int recoverDataRc  = _pDTLS->recoverData (pBuf, bufSize, &pBuf);
    checkAndLogMsg (pszMethodName, Logger::L_HighDetailDebug,
                    "Recover data rc is %d\n", recoverDataRc);
    ...
    if (_bIsServer || !(handshakeStep == 1)) {
        for (bool bSend = true; bSend; ) {
            ...
            if ((sendToRc = _pCI->sendTo (pRemoteAddr, pBuf, handshakeRc, 0)) < 0) {
                if (_pCI->isRecoverableSocketError() < 0) {
                    checkAndLogMsg (pszMethodName, Logger::L_MildError,
                        "send to <%s>:<%hu> failed; with unrecoverable error\n", pRemoteAddr->getIPAsString(),
                        pRemoteAddr->getPort());
                    return -2;
                }
                checkAndLogMsg (pszMethodName, Logger::L_MildError,
                                "send to <%s>:<%hu> failed; rc = %d; trying again...\n", pRemoteAddr->getIPAsString(),
                                pRemoteAddr->getPort(), sendToRc);
                continue;
            }
            checkAndLogMsg (pszMethodName, Logger::L_MediumDetailDebug,
                            "Sent: %d bytes to %s:<%hu>\n", sendToRc,
...
{
    const char * const pszMethodName = "DTLSCommInterface::doHandshake";
    int rc;
    int read = 0;
    int counter = 0;
...
int DTLSCommInterface::receive (void * pBuf, int iBufSize, InetAddr * pRemoteAddr)
{
    ...
    const char * const pszMethodName = "DTLSCommInterface::receive";
    if (!_pDTLS->isHandshakeOver()) {
        if (!doHandshake (pRemoteAddr)) {
            ...
            if (!_pDTLS->isHandshakeOver()) {
                checkAndLogMsg (pszMethodName, Logger::L_Warning,
                                "Something is wrong with the internal state of the DTLS object\n");
                return -2;
            }
            return 0;
        }
...
{
    const char * const pszMethodName = "DTLSCommInterface::resetDtlsObject";
    checkAndLogMsg (pszMethodName, Logger::L_HighDetailDebug,
                    "Resetting the dtls object of the server mocket\n");
...
{
    const char * const pszMethodName = "DTLSCommInterface::sendHSAgain";
    checkAndLogMsg (pszMethodName, Logger::L_HighDetailDebug,
                    "Sending again the last handshake message to %s\n",
                    _pLastRemoteAddr->getIPAsString());
    return _pCI->sendTo (_pLastRemoteAddr, _pcLastHanshakeMessage, _lastMessageSize, nullptr);
}

int DTLSCommInterface::sendTo (InetAddr * pRemoteAddr, const void * pBuf, int iBufSize, const char * pszHints)
{
    static const char * const pszMethodName = "DTLSCommInterface::sendTo";
    int rc = 0;
    ...
    if (!(rc = _pDTLS->isHandshakeOver())) {
        ...
        iSize = _pDTLS->prepareDataForSending (reinterpret_cast<const char *> (pBuf), iBufSize, &pEMsg);
        static const char *pszMessage2 = "Original size: %d; Send message size: %d\n";
        checkAndLogMsg(pszMethodName, Logger::L_HighDetailDebug, pszMessage2, iBufSize, iSize);
        if (pEMsg == nullptr) {
            static const char *pszMessage3 = "Message enctyption failed\n";
            checkAndLogMsg (pszMethodName, Logger::L_MildError, pszMessage3);
        }
        iSize = _pCI->sendTo (pRemoteAddr, pEMsg, iSize, pszHints);
        delete[] pEMsg;
        return iSize;
    }
}
...
{
    if (pszPathToCertificate != nullptr) {
    ...
    if (pszPathToPrivateKey != nullptr) {
...
bool DTLSCommInterface::setLastHanshakeMessage (const char * buffer, const int size, const InetAddr * pRemoteAddr)
{
    const char * const pszMethodName = "DTLSCommInterface::setLastHanshakeMessage";
    checkAndLogMsg (pszMethodName, Logger::L_LowDetailDebug,
                    "Storing current handshake message\n");
    ...
    if ((_pcLastHanshakeMessage != nullptr) && (_pLastRemoteAddr != nullptr)) {
...
{
    const char * const pszMethodName = "DTLSCommInterface::startHandshake";
    ...
    if ((rc = _pCI->sendTo (pRemoteAddr, pBuf, read, 0) < 0)) {
        checkAndLogMsg (pszMethodName, Logger::L_MildError,
                        "Client failed to send HS packet to %s:%d; rc = %d\n",
                        pRemoteAddr->getIPAsString(), (int)pRemoteAddr->getPort(), rc);
    ...
    pBuf = nullptr;
    checkAndLogMsg (pszMethodName, Logger::L_HighDetailDebug,
                    "First message sent <%B>\n", read);
    return true;
}

int DTLSCommInterface::isRecoverableSocketError (void) 
{
    return _pCI->isRecoverableSocketError();
}