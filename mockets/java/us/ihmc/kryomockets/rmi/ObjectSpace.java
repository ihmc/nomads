...
import us.ihmc.kryomockets.EndPoint;
import us.ihmc.kryomockets.FrameworkMessage;
import us.ihmc.kryomockets.Listener;

import static com.esotericsoftware.minlog.Log.*;

/**
 * Allows methods on objects to be invoked remotely over TCP. Objects are {@link #register(int,
 * Object) registered} with an ID.
 ...
 * a return
 * value which is not {@link RemoteObject#setNonBlocking(boolean) ignored}, an extra byte is written. If the type of
 * a parameter is
...
public class ObjectSpace
{
    ...
    private final Listener invokeListener = new Listener()
    {
        public void received (final Connection connection, Object object)
        {
            ...
            if (target == null) {
                if (WARN)
                    warn("kryonet", "Ignoring remote invocation request for unknown object ID: " + invokeMethod
                            .objectID);
                return;
            }
            if (executor == null)
    ...
    /**
     * Creates an ObjectSpace with no connections. Connections must be {@link #addConnection(Connection) added} to
     * allow the remote
    ...
    /**
     * Creates an ObjectSpace with the specified connection. More connections can be {@link #addConnection
     * (Connection) added}.
     */
    ...
    /**
     * Sets the executor used to invoke methods when an invocation is received from a remote endpoint. By default,
     * no executor is
     ...
     * <p/>
     * If a connection is added to multiple ObjectSpaces, the same object ID should not be registered in more than
     * one of those
     * ObjectSpaces.
     *
    ...
    /**
     * Invokes the method on the object and, if necessary, sends the result back to the connection that made the
     * invocation
    ...
    protected void invoke (Connection connection, Object target, InvokeMethod invokeMethod)
    {
        if (DEBUG) {
            ...
            debug("kryonet", connection + " received: " + target.getClass().getSimpleName() + "#" + invokeMethod
                    .method.getName()
                    + "(" + argString + ")");
        }
        ...
        int length = connection.sendMockets(invokeMethodResult);
        if (DEBUG) debug("kryonet", connection + " sent: " + result + " (" + length + ")");
    }

    /**
     * Identical to {@link #getRemoteObject(Connection, int, Class...)} except returns the object cast to the
     * specified interface
    ...
    /**
     * Returns a proxy object that implements the specified interfaces. Methods invoked on the proxy object will be
     * invoked
     * remotely on the object with the specified ID in the ObjectSpace for the specified connection. If the remote
     * end of the
     * connection has not {@link #addConnection(Connection) added} the connection to the ObjectSpace,
     * the remote method invocations
     * will be ignored.
     * <p/>
     * Methods that return a value will throw {@link TimeoutException} if the response is not received with the
     * {@link RemoteObject#setResponseTimeout(int) response timeout}.
     * <p/>
     * If {@link RemoteObject#setNonBlocking(boolean) non-blocking} is false (the default),
     * then methods that return a value must
     * not be called from the update thread for the connection. An exception will be thrown if this occurs. Methods
     * with a void
     * return value can be called on the update thread.
     * <p/>
     * If a proxy returned from this method is part of an object graph sent over the network,
     * the object graph on the receiving
        ...
        public Object invoke (Object proxy, Method method, Object[] args) throws Exception
        {
            if (method.getDeclaringClass() == RemoteObject.class) {
                ...
                // Should never happen, for debugging purposes only
                    ...
                    throw new RuntimeException("Invocation handler could not find RemoteObject method. Check " +
                            "ObjectSpace.java");
                }
            }
            ...
            int length = connection.sendMockets(invokeMethod);
            if (DEBUG) {
                String argString = "";
                if (args != null) {
                ...
                debug("kryonet", connection + " sent: " + method.getDeclaringClass().getSimpleName() + "#" + method
                        .getName() + "("
                        + argString + ") (" + length + ")");
            }
            ...
            try {
                ...
                throw new TimeoutException("Response timed out: " + method.getDeclaringClass().getName() + "." +
                        method.getName());
            }
        }
    ...
    static public class InvokeMethod implements FrameworkMessage, KryoSerializable
    {
        ...
        public Object[] args;
        // The top two bytes of the ID indicate if the remote invocation should respond with return values and
        // exceptions,
        // respectively. The rest is a six bit counter. This means up to 63 responses can be stored before undefined
        // behavior
