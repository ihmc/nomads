...
public class MultiThreadTest
{
    ...
    {
        ConnHandler (MessageMocket mocket, boolean sequenced, boolean reliable)
        {
            _mocket = mocket;
            try {
        ...
        public void run()
        {
            try {
                ...
                if (_useMockets) {
                    ...
                    _mocket.addStatusListener(this);
                }
                else {
        ...
        public void run()
        {
            ...
            try {
                ...
                if (_useMockets) {
                    ...
                    MessageMocket mocket = new MessageMocket();
                    mocket.connect (remoteAddr, CONNECT_TIMEOUT);
                    mocket.addStatusListener (this);
                    Sender sender = mocket.getSender (_sequenced, _reliable);
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        if (server) {
            if (!useSockets) {
                MessageServerMocket sm = new MessageServerMocket (portNumber);
                System.out.println ("MultiThreadTest :: created a MessageServerMocket on port " + portNumber);

                while (true) {
                    try {
                        MessageMocket mocket = sm.accept();
            ...
            for (int j = 0; j < NUM_TRIES; ++j) {
                ...
                if (!useSockets) {
                    System.out.println ("MultiThreadTest :: connecting a MessageMocket to " + remoteHost + " port " + portNumber);
    ...
    synchronized static void saveStats (long id, MessageMocket mocket, String type, long txTime)
    {
        ...
        try {
            ...
            MessageMocket.Statistics stats = mocket.getStatistics();
