...
void ConnHandler::run (void)
{
    ...
    while (true) {
        ...
        if ((i = _pMocket->receive (buf, sizeof (buf))) > 0) {
            printf ("Server: Read data. Iteration %d\n", read);
            read++;
        }
        else {
...
//**// CLIENT  SENDER -calls suspend and getState-
    if (_bSender) {
        ...
        for (int c=0; c<15; c++) {
           ...
           printf ("Read data: %d byte. Iteration %d\n", receive, c);
        }

        //!!// Send data
        for (int i = 0; i < (int) ui16Iterations; i++) {
            printf ("doClientTask: Sending data... Iteration %d\n", i);
...
//**// CLIENT RECEIVER -calls resumeAndRestoreState-
    ...
    //**// CLIENT second RECEIVER -calls resumeAndRestoreState-
    else if (_bReceiver2) {
