 ...
 * This test is meant to be run together with the client side: SimultaneousFreezeDefrost
 * This test performs a mocket migration on the server side.
 * Start a server receiver (-server -receiver)
 * Start a server sender (-client -sender)
*/
...
void ConnHandler::run (void)
{
    if (_bSender) {
        ...
        int suspend_res;
        suspend_res = _pMocket->suspend(0, 5000);
        while (suspend_res != 0) {
