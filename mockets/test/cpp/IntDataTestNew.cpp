...
class ServerMocketDtlsThread : public Thread
{
    public:
        ServerMocketDtlsThread (int usServerPort);
        void run (void);

    private:
        ServerMocket *_pServerMocketDtls;
};
...
{
    public:
        ConnHandler (Mocket *pMocket,bool bDtls);
...
ServerMocketThread::ServerMocketThread (int usServerPort)
{
    ...
    if (pConfigFile != NULL) {
       ...
       _pServerMocket = new ServerMocket(0, 0, false, false);
    }
    _pServerMocket->listen (usServerPort);
    printf("Listen on port %d\n",usServerPort);
}

ServerMocketDtlsThread::ServerMocketDtlsThread (int usServerPort)
{
    const char szConfigFile[] = "mockets.conf";
    FILE *pConfigFile = fopen (szConfigFile, "r");
    if (pConfigFile != NULL) {
        fclose (pConfigFile);
        _pServerMocketDtls = new ServerMocket (szConfigFile);
        printf ("Using config file %s Waring To fix!!!\n", szConfigFile);
    }
    else {	
       _pServerMocketDtls = new ServerMocket(0, 0, false, true);
    }
    _pServerMocketDtls->listen (usServerPort);
    printf("MocketsDtls Listen on port %d\n",usServerPort);
}
...
void ServerMocketThread::run (void)
{
    while (true) {
        ...
        if (pMocket) {
            ...
            ConnHandler *pHandler = new ConnHandler (pMocket,false);
            pHandler->start();
        }
    }
}
void ServerMocketDtlsThread::run (void)
{
    while (true) {
        Mocket *pMocketDtls = _pServerMocketDtls->accept();
        if (pMocketDtls) {
            printf ("ServerMocketDtls: got a connection\n");
            ConnHandler *pHandler = new ConnHandler (pMocketDtls,true);
            pHandler->start();
        }
    }
}
...
void ServerUDTThread::run (void)
{
    ...
    printf ("***ServerUDTThread***After accept UDT connection");
}

ConnHandler::ConnHandler (Mocket *pMocket, bool bDtls)
{
	_pMocket = pMocket;
	if (bDtls) {
		_useMockets = false;
		_useMocketsDtls = true;
	}
	else {
		_useMockets = true;
		_useMocketsDtls = false;
	}
    _pSocket = NULL;
    _pSCTPSocket = NULL;
...
void ConnHandler::run (void)
{
    ...
    if (_useMocketsDtls) {
/*The code below regards the MocketDtls test*/
        printf ("ConnHandler::run: client handler thread started for an incoming mocketsDtls connection\n");
		_pMocket->setIdentifier("IntDataTest-ServerDtls");
		_pMocket->registerPeerUnreachableWarningCallback(unreachablePeerCallback, NULL);
		if (sizeof(iBytesToRead) != _pMocket->receive(&iBytesToRead, sizeof(iBytesToRead))) {
            printf ("ConnHandler::run: receive failed to read size of data; terminating\n");
			_pMocket->close();
        ...
        iBytesToRead = EndianHelper::ntohl ((uint32)iBytesToRead);
        printf ("ConnHandler::run: will read [%d] bytes from client\n", iBytesToRead);
        _pSCTPSocket->send (&chReply, sizeof (chReply));

        i64StartTime = getTimeInMilliseconds();
        while (iBytesRead < iBytesToRead) {
            if ((i = _pSCTPSocket->receive (buf, sizeof (buf))) > 0) {
                iBytesRead += i;
            }
            else {
                printf ("ConnHandler::run: receive returned %d; closing connection\n", i);
        ...
        iBytesToRead = EndianHelper::ntohl ((uint32) iBytesToRead);
        printf ("ConnHandler::run: will read [%d] bytes from client\n", iBytesToRead);
        if (UDT::ERROR == (i = UDT::sendmsg (_udtSocket, &chReply, sizeof (chReply), -1, true))) {
            printf ("Error in UDT receiving the size of the data to read: %s\n", UDT::getlasterror().getErrorMessage());
            UDT::close (_udtSocket);
            return;
        }
        ...
        i64StartTime = getTimeInMilliseconds();
        while (iBytesRead < iBytesToRead) {
            if (UDT::ERROR == (i = UDT::recvmsg (_udtSocket, buf, sizeof (buf)))) {
                printf ("Error in UDT receiving the size of the data to read: %s\n", UDT::getlasterror().getErrorMessage());
                break;
            }
            iBytesRead += i;
...
ClientTaskThread::ClientTaskThread (uint16 ui16ThreadID, const char * const pszRemoteHost, unsigned short usRemotePort, const char* pProtocol, bool bEnableKeyExchange,
                                    uint32 ui32SizeInKB, uint16 ui16MessageSizeInBytes, Statistics * const pStats, AtomicVar<double> &avThroughputStats) :
    _ui16ThreadID(ui16ThreadID), _pszRemoteHost(pszRemoteHost), _usRemotePort(usRemotePort), _pProtocol(pProtocol), _bEnableKeyExchange(bEnableKeyExchange),
...
void ClientTaskThread::run (void)
{
    ...
    if (tp < 0) {
        ...
        if (0 == stricmp (_pProtocol, "mocketsDtls")) {
           fprintf(stderr, "Thread# %hu: client task using %s has failed\n", _ui16ThreadID, _pProtocol);
        }
...
double doClientTask (const char *pszRemoteHost, unsigned short usRemotePort, const char *pProtocol, bool bEnableKeyExchange, uint32 ui32SizeInKB, uint16 ui16MessageSizeInBytes, Statistics *pStats)
{
    ...
    if (0 == stricmp (pProtocol, "mocketsDtls")) {
        printf ("***Protocol: mocketsDtls\n");
        //Creating client mockets
        Mocket  *pm;
	//Time must include handshake 
        i64StartTime = getTimeInMilliseconds();
        pm = new Mocket(0,0,false,true);
        pm->setIdentifier ("IntDataTest-ClientDtls");
        pm->registerPeerUnreachableWarningCallback (unreachablePeerCallback, NULL);
        const char szConfigFile[] = "mockets.conf";
        if (0 != (rc = pm->connect (pszRemoteHost, usRemotePort, bEnableKeyExchange))) {
            fprintf (stderr, "doClientTask: failed to connect using Mockets to remote host %s on port %d; rc = %d\n",
    ...
    if (0 == stricmp (pProtocol, "udt")) {
        ...
        printf ("****Before created the udt socket\n");
        UDTSOCKET udtClientSocket = UDT::socket (AF_INET, SOCK_DGRAM, 0);
        printf ("****After created the udt socket\n");
        ...
        if (UDT::ERROR == (rc = UDT::recvmsg (udtClientSocket, &chReply, sizeof (chReply)))) {
            printf ("doClientTask: an error occurred while receiving the '.'; %s\n", UDT::getlasterror().getErrorMessage());
            UDT::close (udtClientSocket);
            delete[] buf;
            return -11;
        }
        ...
        while (iBytesSent < ui32DataSize) {
            ...
            iBytesToSend = min ((int) ui16MessageSizeInBytes, (ui32DataSize - iBytesSent));
            #endif
            if (UDT::ERROR == UDT::sendmsg (udtClientSocket, reinterpret_cast<const char *> (buf), iBytesToSend, -1, true)) {
	...
	//UDT::close (udtClientSocket);
        ...
        if (UDT::ERROR == UDT::perfmon (udtClientSocket, &traceInfo)) {
            ...
            fprintf (file, "%lu, %u, %.2f, %lu, %lu, %d, %d, %d\n", (unsigned long) (getTimeInMilliseconds () / 1000),
...
int main (int argc, char *argv[])
{
    ...
    if (bServer) {
       ...
       if ((bMockets) && (bSockets) && (bSCTPSockets) && bUDT) {
            printf ("Creating a ServerMocket, ServerMocketDtls, ServerSocket and SCTPServer on port %hu, and a UDTServerSocket on port %hu\n", ui16Port, ui16Port + 2);
	    ServerMocketThread pSMT(ui16Port);
	    pSMT.start();

	    ServerMocketDtlsThread pSMTDtls(ui16Port + 1);
	    pSMTDtls.start();
        ...
        pSMT.start();
		printf("Starting Mockets Dtls Server thread\n");
        ServerMocketThread pSMTDtls(ui16Port + 1);
		printf("ServerMocketDtlsThread\n");
        pSMTDtls.run();
	}
       if ((!bMockets) && (bSockets) && (!bSCTPSockets) && !bUDT) {
		ServerSocketThread pSST(ui16Port);
       ...
       if (bUDT) {
        ...
        for (int i = 0; i < (int) ui16Iterations; i++) {
            ...
            if (bMockets) {
				...
				printf("Done with Mockets Tests - Sleeping for %d seconds before starting Mockets Dtls Test...\n\n", ui16SecToSleep);
				sleepForMilliseconds(ui16SecToSleep * 1000);


				// Create threads
				vector<ClientTaskThread> threadVectorDtls;
				for (int j = 0; j < ui16ConcurrentFlows; j++) {
					threadVectorDtls.push_back(ClientTaskThread((i * ui16ConcurrentFlows) + j, remoteHost, ui16Port + 1, "mocketsDtls", bEnableKeyExchange,
						ui32SizeInKB, ui16MessageSizeInBytes, &mocketStats, dMocketThroughput));
				}
				printf("Starting MocketDtls Test...\n");
				ui64OverallMocketStartTime = getTimeInMilliseconds();
				// Start threads
				for (vector<ClientTaskThread>::iterator iter = threadVectorDtls.begin(); iter != threadVectorDtls.end(); iter++) {
					(*iter).start(false);
				}
				// Join threads before moving on to the next iteration
				for (vector<ClientTaskThread>::iterator iter = threadVectorDtls.begin(); iter != threadVectorDtls.end(); iter++) {
            ...
            if (bSCTPSockets) {
                ...
                if (bUDT) {
                    printf ("Done with SCTPSockets Tests - Sleeping for %d seconds before starting UDT Test...\n\n", ui16SecToSleep);
                    sleepForMilliseconds (ui16SecToSleep * 1000);
                }
            }
