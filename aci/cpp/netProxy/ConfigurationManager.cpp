...
namespace ACMNetProxy
{
    int NetProxyConfigManager::EndpointConfigParameters::parse (char *pszParamsEntry)
    {
        ...
        while ((sKeyValuePair = st.getNextToken()).length() > 0) {
            ...
            if (sKey.length() <= 0) {
                checkAndLogMsg ("NetProxyConfigManager::EndpointConfigParameters::parse", Logger::L_Warning,
            ...
            if (sKey == "icmp") {
                if (!ProtocolSetting::isProtocolNameCorrect (sValue)) {
                    ...
                    checkAndLogMsg ("NetProxyConfigManager::EndpointConfigParameters::parse", Logger::L_LowDetailDebug,
				...
				_psTCPProtocolSetting.setPriorityLevel (readPriorityConfEntry (sValue));
				checkAndLogMsg ("NetProxyConfigManager::EndpointConfigParameters::parse", Logger::L_LowDetailDebug,
					            "TCP priority level set to %d\n", _psTCPProtocolSetting.getPriorityLevel());
            }
            else if (sKey == "tcpencryption") {
                EncryptionType etTCP = readEncryptionValue (sValue);
                if (etTCP == ET_UNDEF) {
                    checkAndLogMsg ("NetProxyConfigManager::EndpointConfigParameters::parse", Logger::L_Warning,
                ...
                EncryptionType etUDP = readEncryptionValue (sValue);
                if (etUDP == ET_UNDEF) {
                    checkAndLogMsg ("NetProxyConfigManager::EndpointConfigParameters::parse", Logger::L_Warning,
                                    "Unable to parse the string %s to set the encryption protocol for "
                ...
                EncryptionType etICMP = readEncryptionValue (sValue);
                if (etICMP == ET_UNDEF) {
                    checkAndLogMsg ("NetProxyConfigManager::EndpointConfigParameters::parse", Logger::L_Warning,
                                    "Unable to parse the string %s to set the encryption protocol for "
	...
	const uint8 NetProxyConfigManager::EndpointConfigParameters::readPriorityConfEntry (const char * const pcPriority)
	{
		int iPriorityLevel = ProtocolSetting::getDefaultTCPProtocolSetting()->getPriorityLevel();

		if (pcPriority) {
			iPriorityLevel = atoi (pcPriority);
			if (iPriorityLevel < 0) {
                checkAndLogMsg ("NetProxyConfigManager::EndpointConfigParameters::readPriorityConfEntry", Logger::L_Warning,
    ...
    int NetProxyConfigManager::AddressMappingConfigFileReader::parseAndAddEntry (const char *pszEntry)
    {
        ...
        if (P_CONNECTION_MANAGER->addNewAddressMappingToBook (rangeDescriptor, ui32RemoteProxyID) < 0) {
            checkAndLogMsg ("NetProxyConfigManager::AddressMappingConfigFileReader::parseAndAddEntry", Logger::L_Warning,
    ...
    int NetProxyConfigManager::UniqueIDsConfigFileReader::parseAndAddEntry (const char *pszEntry)
    {
        ...
        remoteProxyInfo.setMocketsConfigFileName (NetProxyApplicationParameters::DEFAULT_MOCKETS_CONFIG_FILE);        // Set the default Mockets Configuration file
        ...
        checkAndLogMsg ("NetProxyConfigManager::UniqueIDsConfigFileReader::parseAndAddEntry", Logger::L_Info,
                        ...
                        pRemoteProxyInfo->getRemoteProxyInetAddr (CT_TCPSOCKET)->getPort(), pRemoteProxyInfo->getRemoteProxyInetAddr (CT_UDPSOCKET)->getPort());
    ...
    {
        if ((sKey == nullptr) || (sValue == nullptr)) {
            return -1;
        }
        ...
        if (sKey == "mocketsport") {
            ...
            if (isConnectorTypeNameCorrect (sValue)) {
                ...
                if (P_CONNECTION_MANAGER->addOrUpdateAutoConnectionToList (autoConnectionEntry) > 0) {
                    checkAndLogMsg ("NetProxyConfigManager::UniqueIDsConfigFileReader::updateInfoWithKeyValuePair", Logger::L_Info,
                                    "successfully added a new entry to the AutoConnection Table for the NetProxy with UniqueID %u and address "
                                    "%s using protocol %s and a connection attempt timeout of %ums\n", remoteProxyInfo.getRemoteProxyID(),
                                    remoteProxyInfo.getRemoteProxyIPAddressAsString(), connectorTypeToString (autoConnectionEntry.getConnectorType()),
            ...
            if (pAutoConnectionEntry) {
                ...
                checkAndLogMsg ("NetProxyConfigManager::UniqueIDsConfigFileReader::updateInfoWithKeyValuePair", Logger::L_Info,
                                ...
                                remoteProxyInfo.getRemoteProxyID(), remoteProxyInfo.getRemoteProxyIPAddressAsString(), ui32ReconnectTime);
            }
            else {
                checkAndLogMsg ("NetProxyConfigManager::UniqueIDsConfigFileReader::updateInfoWithKeyValuePair", Logger::L_Info,
            ...
            if (sValue == ACTIVE_CONNECTIVITY_CONFIG_PARAMETER) {
                ...
                AutoConnectionEntry * const pAutoConnectionEntry = P_CONNECTION_MANAGER->getAutoConnectionEntryForProxyWithID (remoteProxyInfo.getRemoteProxyID());
                if (pAutoConnectionEntry) {
                    pAutoConnectionEntry->setInvalid (ET_UNDEF);
    ...
    int NetProxyConfigManager::UniqueIDsConfigFileReader::updateInfoWithMocketsConfigFilePath (RemoteProxyInfo & remoteProxyInfo, const String & sMocketsConfigFilePath)
    {
        ...
        remoteProxyInfo.setMocketsConfigFileName (sMocketsConfigFilePath);
    ...
    int NetProxyConfigManager::StaticARPTableConfigFileReader::parseAndAddEntry (const char * const pszEntry) const
    {
        ...
        if (pLocalIPAddress == nullptr) {
            return -2;
        }
        // Get Ethernet MAC address
        const char * const pLocalEtherMACAddress = st.getNextToken();
        if (pLocalEtherMACAddress == nullptr) {
            return -3;
        }
    ...
    int NetProxyConfigManager::StaticARPTableConfigFileReader::parseEtherMACAddress(const char * const pcEtherMACAddressString, EtherMACAddr * const pEtherMACAddress) const
    {
        ...
        unsigned int i = 0;
        if ((pszMACAddressOctets[i++] = strtok_mt (pcEtherMACAddressString, AC_SEPARATOR, &pszTemp)) == nullptr) {
            return -2;
        }
        for (; i < MAC_ADDRESS_SIZE; ++i) {
            pszMACAddressOctets[i] = strtok_mt (nullptr, AC_SEPARATOR, &pszTemp);
    ...
    int NetProxyConfigManager::processConfigFiles (void)
    {
        ...
        if (pszAddressMappingConfigFile != nullptr) {
            ...
            if (0 != (rc = _amcfr.parseAddressMappingConfigFile (szFile))) {
                checkAndLogMsg ("NetProxyConfigManager::processConfigFiles", Logger::L_Warning,
	...
	{
		int rc;
        PacketRouter * const pPacketRouter = PacketRouter::getPacketRouter();
        ...
        if (hasValue ("ExternalIPAddress")) {
            // IP Address of the external interface
            String sExternalInterfaceIPAddress = getValue ("ExternalIPAddress");
            if (sExternalInterfaceIPAddress.length() > 0) {
                InetAddr externalInterfaceInetAddr(sExternalInterfaceIPAddress);
                NetProxyApplicationParameters::EXTERNAL_IP_ADDR = externalInterfaceInetAddr.getIPAddress();
                checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                                "IP Address of the %s interface set to %s\n",
                                NetProxyApplicationParameters::GATEWAY_MODE ?
                                "external network" : "virtual TUN/TAP",
                                externalInterfaceInetAddr.getIPAsString());
            }
        }
        else {
            checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Warning,
                            "No value found for the IP address of the %s interface; "
                            "NetProxy will try to query it from the adapter\n",
                            NetProxyApplicationParameters::GATEWAY_MODE ?
                            "external network" : "virtual TUN/TAP");
        }
		...
		if (NetProxyApplicationParameters::GATEWAY_MODE) {
            ...
            // Configured value for the network netmask of the external interface
			if (hasValue ("ExternalIPNetmask")) {
				NetProxyApplicationParameters::EXTERNAL_NETWORK_NETMASK = InetAddr(getValue ("ExternalIPNetmask")).getIPAddress();
				checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
					            "Network netmask of the external interface set to: %s\n",
					            InetAddr(NetProxyApplicationParameters::EXTERNAL_NETWORK_NETMASK).getIPAsString());
			}
			else {
				checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Warning,
			...
			if (hasValue ("GatewayAddress")) {
				NetProxyApplicationParameters::NETWORK_GATEWAY_IP_ADDR = InetAddr(getValue ("GatewayAddress")).getIPAddress();
				checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
            ...
            // IP Address of the internal or TUN/TAP interface
            if (hasValue ("InternalIPAddress")) {
                String sInternalInterfaceIPAddress = getValue ("InternalIPAddress");
                if (sInternalInterfaceIPAddress.length() > 0) {
                    InetAddr internalInterfaceInetAddr(sInternalInterfaceIPAddress);
                    NetProxyApplicationParameters::INTERNAL_IP_ADDR = internalInterfaceInetAddr.getIPAddress();
                    checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                                    "IP Address of the internal network interface set to %s\n",
                                    internalInterfaceInetAddr.getIPAsString());
                }
            }
            else {
                checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Warning,
                                "No value found for the IP address of the internal network interface; "
                                "NetProxy will try to query it from the adapter\n");
            }

            // Configured value for the network netmask of the internal interface
            if (hasValue ("InternalIPNetmask")) {
                NetProxyApplicationParameters::INTERNAL_NETWORK_NETMASK = InetAddr(getValue ("ExternalIPNetmask")).getIPAddress();
                checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                                "Network netmask of the internal interface set to: %s\n",
                                InetAddr(NetProxyApplicationParameters::INTERNAL_NETWORK_NETMASK).getIPAsString());
            }
            else {
                checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Warning,
                                "Impossible to find a value for the network netmask of the internal interface; "
                                "NetProxy will try and query it from the adapter\n");
            }
		}
		else {
			// Check if the MTU of the TAP Interface has not been retrieved, yet
			if (NetProxyApplicationParameters::ETHERNET_MTU_INTERNAL_IF == 0) {
				if (hasValue ("TAPInterfaceMTU")) {
					// Value of the MTU of the TUN/TAP Interface
					int iTAPInterfaceMTU = getValueAsInt ("TAPInterfaceMTU");
					if ((iTAPInterfaceMTU >= NetProxyApplicationParameters::ETHERNET_MIN_MTU) &&
						(iTAPInterfaceMTU <= NetProxyApplicationParameters::ETHERNET_MAX_MTU)) {
						NetProxyApplicationParameters::ETHERNET_MTU_INTERNAL_IF = static_cast<uint16> (iTAPInterfaceMTU);
						checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
							            "MTU of the TUN/TAP interface set to %hu bytes\n",
							            NetProxyApplicationParameters::ETHERNET_MTU_INTERNAL_IF);
					}
					else {
					...
					NetProxyApplicationParameters::ETHERNET_MTU_INTERNAL_IF = NetProxyApplicationParameters::TAP_INTERFACE_DEFAULT_MTU;
					checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
        ...
        const char *pARPMissCacheTimeout = getValue ("ARPMissCacheTimeout");
        if (pARPMissCacheTimeout) {
            int64 i64ARPMissCacheTimeout = atoi (pARPMissCacheTimeout);
            if (i64ARPMissCacheTimeout <= 0) {
                ...
                pPacketRouter->_ARPTableMissCache.setTimeout (NetworkConfigurationSettings::ARP_TABLE_MISS_EXPIRATION_TIME_IN_MS);
                checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
        ...
        // Forwarding of packets with a Multicast/Broadcast MAC destination address on the external/internal interface
        if (hasValue ("forwardMulticastPacketsOnExternalInterface")) {
            ...
            NetProxyApplicationParameters::MULTICAST_PACKETS_FORWARDING_ON_EXTERNAL_NETWORK = bMCastPacketForwarding;
        }
        checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                        "Forwarding of Multicast MAC packets on the external interface is %s\n",
                        NetProxyApplicationParameters::MULTICAST_PACKETS_FORWARDING_ON_EXTERNAL_NETWORK ?
                        "enabled" : "disabled");

        if (hasValue ("forwardMulticastPacketsOnInternalInterface")) {
            bool bMCastPacketForwarding = getValueAsBool ("forwardMulticastPacketsOnInternalInterface");
            NetProxyApplicationParameters::MULTICAST_PACKETS_FORWARDING_ON_INTERNAL_NETWORK = bMCastPacketForwarding;
        }
        checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                        "Forwarding of Multicast MAC packets on the internal interface is %s\n",
                        NetProxyApplicationParameters::MULTICAST_PACKETS_FORWARDING_ON_INTERNAL_NETWORK ?
                        "enabled" : "disabled");
        ...
        if (hasValue ("forwardBroadcastPacketsOnExternalInterface")) {
            ...
            NetProxyApplicationParameters::BROADCAST_PACKETS_FORWARDING_ON_EXTERNAL_NETWORK = bBCastPacketForwarding;
        }
        checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                        "Forwarding of Broadcast MAC packets on the external interface is %s\n",
                        NetProxyApplicationParameters::BROADCAST_PACKETS_FORWARDING_ON_EXTERNAL_NETWORK ?
                        "enabled" : "disabled");

        if (hasValue ("forwardBroadcastPacketsOnInternalInterface")) {
            bool bBCastPacketForwarding = getValueAsBool ("forwardBroadcastPacketsOnInternalInterface");
            NetProxyApplicationParameters::BROADCAST_PACKETS_FORWARDING_ON_INTERNAL_NETWORK = bBCastPacketForwarding;
        }
        checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                        "Forwarding of Broadcast MAC packets on the internal interface is %s\n",
                        NetProxyApplicationParameters::BROADCAST_PACKETS_FORWARDING_ON_INTERNAL_NETWORK ?
                        "enabled" : "disabled");
		...
		if (hasValue ("ActivateNetSensor")) {
            ...
            checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                            "ActivateNetsensor field not found; NetProxy will run NetSensor as per default option\n");
			NetProxyApplicationParameters::ACTIVATE_NETSENSOR = true;
		}

        if (hasValue ("netsensor_stat_destination")) {
            NetProxyApplicationParameters::NETSENSOR_STAT_DEST_IP = getValue("netsensor_stat_destination");
            checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                            "netsensor_stat_destination option set as %s\n",
                            NetProxyApplicationParameters::NETSENSOR_STAT_DEST_IP.c_str());
        }
        else {
            checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                            "netsensor_stat_destination field not found; sending to local host\n");
            NetProxyApplicationParameters::NETSENSOR_STAT_DEST_IP = "localhost";
        }

		if (hasValue ("priority_mechanism")) {
			NetProxyApplicationParameters::PRIORITY_MECHANISM = getValueAsBool ("priority_mechanism");
			if (NetProxyApplicationParameters::PRIORITY_MECHANISM) {
				checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
					            "Priority mechanism set as active\n");
			}
			else {
				checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
					            "Priority mechanism set as not active\n");
			}
		}
		else {
            checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                            "Priority_mechanism option not found; function will be disabled by default. "
                            "To enable it, please add the entry <PRIORITY_MECHANISM = true> in the configuration file\n");
			NetProxyApplicationParameters::PRIORITY_MECHANISM = false;
		}
        ...
        checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                        ...
                        NetProxyApplicationParameters::UPDATE_GUI_THREAD_ENABLED ?
                        "enabled" : "disabled");
        ...
        if (NetProxyApplicationParameters::UPDATE_GUI_THREAD_ENABLED) {
            ...
            while (const char * const pszPairToken = stAddressList.getNextToken()) {
                ...
                if (!InetAddr::isIPv4Addr (sIP)) {
                    checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Warning,
                                    "entry <%s> in the StatusNotificationAddresses list does not contain a valid IP address; using "
                                    "the default value %s\n", pszPairToken, NetProxyApplicationParameters::DEFAULT_GUI_IP_ADDR);
                    sIP = NetProxyApplicationParameters::DEFAULT_GUI_IP_ADDR;
                }
                if (sPort.length() <= 0) {
                    checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Info,
                                    "port not specified for address %s in the StatusNotificationAddresses list; using the "
                                    "default port %hu\n", sIP.c_str(), NetProxyApplicationParameters::DEFAULT_GUI_PORT);
                    ui32GUIPortNumber = NetProxyApplicationParameters::DEFAULT_GUI_PORT;
                }
                else {
                    // Parsing port number
                    ...
                    if (ui32GUIPortNumber > 65535U) {
                        checkAndLogMsg ("NetProxyConfigManager::processNetProxyConfigFile", Logger::L_Warning,
                                        "%u is not an acceptable port number; the default value %hu will be used for notifications sent to %s\n",
                                        ui32GUIPortNumber, NetProxyApplicationParameters::DEFAULT_GUI_PORT, sIP.c_str());
                        ui32GUIPortNumber = NetProxyApplicationParameters::DEFAULT_GUI_PORT;
                    }
                }
    ...
    void NetProxyConfigManager::consolidateConfigurationSettings (void)
    {
        const NPDArray2<std::pair<AddressRangeDescriptor, ConnectivitySolutions*> > * const remoteHostAddressMappingBook =
            P_CONNECTION_MANAGER->getRemoteHostAddressMappingBook();

        for (int i = 0; i < _ecfr._endpointConfigTable.size(); ++i) {
            for (int j = 0; j < remoteHostAddressMappingBook->size(); ++j) {
                EndpointConfigFileReader::EndpointConfigEntry &pEntry = _ecfr._endpointConfigTable.get(i);
                if (pEntry.matchesAsDestination (remoteHostAddressMappingBook->get (j).first)) {
                    ConnectivitySolutions *pConnectivitySolutions = remoteHostAddressMappingBook->get (j).second;
                    // TCP
                    const ProtocolSetting &tcpSettings = pEntry.getConfigParams()->getProtocolSetting (IP_PROTO_TCP);
                    ...
                    // UDP
                    const ProtocolSetting &udpSettings = pEntry.getConfigParams()->getProtocolSetting (IP_PROTO_UDP);
                    ...
                    // ICMP
                    const ProtocolSetting &icmpSettings = pEntry.getConfigParams()->getProtocolSetting (IP_PROTO_ICMP);
    ...
    int NetProxyConfigManager::splitIPFromPort (String sIPPortPair, char *pcKeyIPAddress, uint16 *pui16KeyPortNumber, bool bAllowWildcards)
    {
        ...
        if (nullptr != (pcTempStr = st.getNextToken())) {
