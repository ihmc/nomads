...
public class URIEncoder {
     ...
     * [RFC1630]). (@link http://www.ietf.org/rfc/rfc1630.txt) Takes all a
     * string's "special characters", characters that are not of type [0-9],
     * [a-z], [A-Z] and converts them to ascii values preceded by a '%'.
     * 
     * @param stringWithSpecialChars The string that will have all the special
     *            characters removed.
     * @return String A String that has has all of the containing special chars
    ...
    public static String includeSpecialChars (String stringWithAsciiChars)
    {
        ...
        for (int init = 0; init < stringLength; init++) {
            newByteArray[init] = 32; // inserts spaces into the new empty
                                        // byte array
        }
    ...
    /**
     * Takes two bytes representing the first and second digit of hex values for
     * a single character and converts them to a single decimal value
