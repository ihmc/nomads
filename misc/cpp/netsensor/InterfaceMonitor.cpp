...
{
int InterfaceMonitor::initLive(const NOMADSUtil::String & sInterfaceName,
                               bool isInternal, bool rttDetection)
{
    ...
    checkAndLogMsg(meName, Logger::L_Info,
                   "Creating new LIVE IMT for: %s\n",
                   sInterfaceName.c_str());

    int msValidity = 1;
    _pNetInterface = PCapInterface::getPCapInterface(sInterfaceName,
                                                     msValidity);
    ...
    _ui32IPAddr = _pNetInterface->getIPv4Addr() ?
		_pNetInterface->getIPv4Addr()->ui32Addr : 0;

    if (_ui32IPAddr == 0) {
        checkAndLogMsg(meName, Logger::L_SevereError,
            "IP address Detection of network interface <%s> failed\n", sInterfaceName.c_str());
    ...
    if ((s = ::socket(PF_INET, SOCK_STREAM, 0)) < 0) {
        checkAndLogMsg(meName,
			Logger::L_SevereError, "Could not obtain socket!\n");
        return -4;
    }
    ...
    if (NULL == (pIPv4IPAddr = _pNetInterface->getDefaultGateway())) {
        checkAndLogMsg(meName, Logger::L_Info,
...
int InterfaceMonitor::initReplay(const NOMADSUtil::String & sInterfaceName,
