...
typedef void (*TerminationCallbackFnPtr)
    (void *pCallbackArg, ManageableThread *pThread);
...
class ManageableThread : public Thread
{
    public:
        ...
        void setTerminationCallback
            (TerminationCallbackFnPtr pCallbackFn, void *pCallbackArg);

    protected:
        // Should be called from the run() method of the thread before doing
        // anything else
        void started                            (void);

        // Should be called from the run() method periodically to check to see
        // if someone has requested termination
        bool terminationRequested               (void);

        // May be used to set a result code before termination
        void setTerminatingResultCode           (int rc);

        // Should be called from the run() method when the method is about to
        // terminate
...
inline void ManageableThread::setTerminationCallback
    (TerminationCallbackFnPtr pCallbackFn, void *pCallbackArg)
{
