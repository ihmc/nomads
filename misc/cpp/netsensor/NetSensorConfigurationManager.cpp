...
int NetSensorConfigurationManager::setReplayModeDefaultConfigurations(const NOMADSUtil::String & statRecipientIP,
...
{
    const char* meName = "printConfigurationReport()";
    checkAndLogMsg(meName, Logger::L_Info, "\tCfg file's version: %d\n", ui32Ver);

    checkAndLogMsg(meName, Logger::L_Info, "\tDelivery Options:\n");

    checkAndLogMsg(meName, Logger::L_Info, "\t\tDelivery MTU is: %d\n", ui32DelMTU);
    checkAndLogMsg(meName, Logger::L_Info, "\t\tIPs that will receive data are:\n");
    printStringList(pslIpDeliveryList);
    checkAndLogMsg(meName, Logger::L_Info, "\t\tDelivery Port is: %d\n", ui32DelPort);
    checkAndLogMsg(meName, Logger::L_Info, "\tTraffic delivery period %dms\n", ui32TDelPer);
    checkAndLogMsg(meName, Logger::L_Info, "\tTopology delivery period %dms\n", ui32ToDelPer);

    checkAndLogMsg(meName, Logger::L_Info, "\tMonitoring Options:\n");

    checkAndLogMsg(meName, Logger::L_Info, "\t\tMonitored interfaces are:\n");
    printStringList(pslMonitoredInterfacesList);

    checkAndLogMsg(meName, Logger::L_Info, "\tARP topology mechanism set to: %d\n", arpTopActive);
    checkAndLogMsg(meName, Logger::L_Info, "\tLax topology mechanism set to: %d\n", laxTopActive);
    checkAndLogMsg(meName, Logger::L_Info, "\tNETMASK topology mechanism set to: %d\n", netmskTopActive);
    checkAndLogMsg(meName, Logger::L_Info, "\tNETPROXY topology mechanism set to: %d\n", nproxyTopActive);
    checkAndLogMsg(meName, Logger::L_Info, "\tICMP RTT detections set to: %d\n", icmpRTTActive);
    checkAndLogMsg(meName, Logger::L_Info, "\tIGNORE Multicast set to: %d\n", ignoreMulticast);
    checkAndLogMsg(meName, Logger::L_Info, "\tStore external nodes for topology set to: %d\n", storeExternalNodes);
    checkAndLogMsg(meName, Logger::L_Info, "\tUse compression for protobuf stream: %d\n", useProtobufCompression);
    checkAndLogMsg(meName, Logger::L_Info, "\tCalculate TCP RTT: %d\n\n\n", calculateTCPRTT);
}
