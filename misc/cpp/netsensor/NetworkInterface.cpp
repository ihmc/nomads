...
* Author: amorelli@ihmc.us rfronteddu@ihmc.us
...
#define BUFFER_SIZE 4096 //change this into const


String NetworkInterface::getDeviceNameFromUserFriendlyName(
	const char * const pszUserFriendlyInterfaceName)
{
#if defined (WIN32)
    String retVal;
    DWORD dwRetVal;
	// Windows API Documentation recommends a 15KB buffer
    ULONG ulFamily = 0, ulFlags = 0, ulOutBufLen = 16384;
    ...
    dwRetVal = GetAdaptersAddresses(ulFamily, ulFlags, NULL, pAddresses,
		&ulOutBufLen);
    if (dwRetVal == ERROR_BUFFER_OVERFLOW) {

        free(pAddresses);

        // Try once again, increasing the buffer size -
		// the needed buffer size is returned back in ulOutBufLen
        ulOutBufLen += 2048;       // Just for good measure
        pAddresses = (IP_ADAPTER_ADDRESSES *)malloc(ulOutBufLen);
        memset(pAddresses, 0, ulOutBufLen);
        dwRetVal = GetAdaptersAddresses(ulFamily, ulFlags, NULL,
			pAddresses, &ulOutBufLen);
    }
    ...
    if (dwRetVal == NO_ERROR) {
        ...
        while (pCurrAddress != NULL) {
            ...
            // GetAdaptersAddresses uses wchar's for the FriendlyName -
...
const uint8 * const NetworkInterface::getMACAddrForDevice(
	const char * const pszAdapterName)
{
#if defined (WIN32)
    ...
    if (dwRetVal == NO_ERROR) {
        ...
        while (pCurrAddress != NULL) {
            ...
            if (sIFAdapterName ^= pszAdapterName) {
                ...
                memcpy(pui8MACAddr, pCurrAddress->PhysicalAddress,
					pCurrAddress->PhysicalAddressLength);
                char szGUID[64];
                sprintf_s(szGUID, 64,
					"{%08x-%04x-%04x-%02X%02X-%02X%02X%02X%02X%02X%02X}",
                    pCurrAddress->NetworkGuid.Data1,
					pCurrAddress->NetworkGuid.Data2,
					pCurrAddress->NetworkGuid.Data3,
                    pCurrAddress->NetworkGuid.Data4[0],
					pCurrAddress->NetworkGuid.Data4[1],
                    pCurrAddress->NetworkGuid.Data4[2],
					pCurrAddress->NetworkGuid.Data4[3],
                    pCurrAddress->NetworkGuid.Data4[4],
					pCurrAddress->NetworkGuid.Data4[5],
    ...
    return pui8MACAddr;
        ...
        if (pIfList == NULL) {
            checkAndLogMsg("NetworkInterface::getMACAddrForDevice",
				Logger::L_MildError, "could not obtain list of interfaces\n");
            return NULL;
        }
        for (pIfListItem = pIfList; pIfListItem->if_index != 0; pIfListItem++) {
            if (0 == stricmp(pIfListItem->if_name, pszAdapterName)) {
                ...
                if (iSocket < 0) {
                    checkAndLogMsg("NetworkInterface::getMACAddrForDevice",
						Logger::L_MildError, "could not create a socket\n");
        ...
        if_freenameindex(pIfList);
        checkAndLogMsg("NetworkInterface::getMACAddrForDevice",
			Logger::L_MildError, "could not find interface %s\n",
			pszAdapterName);
    #endif
        return NULL;
#endif
...
IPv4Addr NetworkInterface::getDefaultGatewayForInterface(
	const char * const pszAdapterName) {
    IPv4Addr res;
    res.ui32Addr = 0;
    ...
    if (!pAdapterInfo) {
        checkAndLogMsg("NetworkInterface::getDefaultGatewayForInterface",
    ...
    if ((dwRetVal = GetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) == NO_ERROR) {
        ...
        while (pAdapter) {
            if (!stricmp(pAdapter->AdapterName, pszAdapterName)) {
                res.ui32Addr =
					InetAddr(pAdapter->GatewayList.IpAddress.String).
					getIPAddress();
                break;
            }
    ...
    if (res.ui32Addr != 0) {
        ...
        checkAndLogMsg("NetworkInterface::getDefaultGatewayForInterface",
			Logger::L_MildError, "Gateway: <not found>\n");
    }
    ...
    if ((sock = socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE)) < 0) {
        checkAndLogMsg("NetworkInterface::getDefaultGatewayForInterface",
			Logger::L_SevereError, "socket() failed with error: %d\n", errno);
        return res;
    }
