...
const uint8 * const NetworkInterface::getMACAddrForDevice(const char * const pszAdapterName)
{
	...
	memset(pAddresses, 0, ulOutBufLen);
	dwRetVal = GetAdaptersAddresses(ulFamily, ulFlags, NULL, pAddresses, &ulOutBufLen);
	if (dwRetVal == ERROR_BUFFER_OVERFLOW){//should not enter here
	...
	if (dwRetVal == NO_ERROR) {
		...
		while (pCurrAddress != NULL) {
			...
			//std::cout << "Possible interface is: " << sIFAdapterName  << std::endl;
			if (sIFAdapterName ^= pszAdapterName) {
				// Found the device
				//std::cout << "Device found" << sIFAdapterName  << std::endl;
...
IPv4Addr NetworkInterface::getDefaultGatewayForInterface(const char * const pszAdapterName) {
	...
	if (res.ui32Addr != 0) {
		checkAndLogMsg("NetworkInterface::getDefaultGatewayForInterface", Logger::L_Info,
