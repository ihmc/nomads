...
/*  ----------  */ 
{
    ...
    ClassNo c;
    float BaseInfo, MinGain, ThisGain, ThisInfo, Val, BestVal, BestGain, BestInfo, PrevVal, PrevGain, PrevInfo;
    short Blocks = 0, MissingValues = 0, ReasonableSubsets, Bytes, b;
    Boolean MergedSubsets = false;
    int SaveMINOBJS;
    SaveMINOBJS = MINOBJS;
    MINOBJS = 1;
    // First compute Freq[][], ValFreq[], base info, and the gain and total info of a split on discrete attribute Att
    ...
    PrevVal = Worth(PrevInfo, PrevGain, (float) Epsilon);
    Verbosity(1)
    {
    ...
    // Eliminate unrepresented attribute values from Freq[] and ValFreq[] and form a separate subset for each represented attribute value
    ...
    // Merge any single-class subsets with others of the same class. Note: have ValFreq[V] > 0 for all V
    ...
    while(Blocks > 2) {
        BestVal = BestV1 = 0;
        BestGain = (float) (-Epsilon);
        // Check reasonable subsets; if less than 3, bar mergers involving the largest block
        ReasonableSubsets = 0;
        Barred = 1;
        ForEach(V1, 1, Blocks) {
        ...
        // For each possible pair of values, calculate the gain and total info of a split in which they are treated as one.
        // Keep track of the pair with the best gain.
        ForEach(V1, 1, Blocks-1) {
            ForEach(V2, V1+1, Blocks) {
                if(V1 == Barred || V2 == Barred) continue;
                Combine(V1, V2, Blocks);
                ThisGain = ComputeGain(BaseInfo, UnknownRate[Att], Blocks-1, KnownItems);
                ThisInfo = TotalInfo(ValFreq, 0, Blocks-1) / Items;
                Val = Worth(ThisInfo, ThisGain, (float) Epsilon);
                Verbosity(1) {
                    printf("\tcombine %d %d info %.3f gain %.3f val %.3f", V1, V2, ThisInfo, ThisGain, Val);
                    PrintDistribution(Att, Blocks-1, false);
                }
                // Force a split if less than two reasonable subsets, or using GAIN criterion
                // Prefer this split to the previous one if gain >= MinGain (and previous < MinGain), or val >= previous best val
                if(ThisGain >= MinGain && BestGain < MinGain || Val >= BestVal || ! BestV1 && ( ! GAINRATIO || ReasonableSubsets < 2 )) {
                    BestVal = Val;
                    BestGain = ThisGain;
                    BestInfo = ThisInfo;
                    BestV1 = V1;
        ...
        if(GAINRATIO && ReasonableSubsets >= 2 && (! BestV1 || BestVal < PrevVal + 1E-5 || BestVal == PrevVal && BestGain < PrevGain)) break;
        PrevGain = BestGain;
        PrevInfo = BestInfo;
        PrevVal = BestVal;
    ...
    if(PrevVal <= 0) {
        ...
        Verbosity(1) {
            ...
            PrintDistribution(Att, Blocks, false);
            printf("\tinf %.3f gain %.3f val %.3f\n", Info[Att], Gain[Att], Worth(Info[Att], Gain[Att], (float)Epsilon));
        }
    }
}
