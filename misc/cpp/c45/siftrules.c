...
ItemNo * Covered;               // Covered[i]   = no. included rules that cover item i
ItemNo * FalsePos;              // FalsePos[c]  = no. false positives from rules selected for class c
ItemNo * NoRule;                // NoRule[c]    = no. items covered by no selected rule
ItemNo * Right;                 // Right[r]     = no. correct rule firings
ItemNo * Wrong;                 // Wrong[r]     = no. incorrect rule firings
float * Value;                  // Value[r]     = advantage attributable to rule r or realisable if rule r included
float SubsetValue;              // value of best class subset so far
float CodeWeight;               // multiplying factor for rule encodings
Boolean	* RuleIn;               // RuleIn[r]    = true if rule r included
Boolean * Subset;               // best class subset so far
Boolean * * Match;              // Match[r][i]  = true if rule r fires on item i
RuleNo * ClassRules;            // list of all rules for current target class
ClassNo	FocusClass;

// functions
void ConstructRuleset(void);
void InitialiseTables(void);
void CoverClass(void);
...
/*  ---------  */
{
    ...
    for(Temp = 1000.0f ; Temp > 0.001f ; Temp *= 0.95f) {
        CalculateValue();
        Verbosity(1) {
            OutCount = 0;
...
/*  --------------  */
{
    ...
    ForEach(r, 1, NRules) {
        if(Rule[r].Rhs == FocusClass) {
            ...
            RuleMatch = Match[r];
            ForEach(i, 0, MaxItem) {
                if(RuleMatch[i] && (!(Times = Covered[i]) || Times == 1 && RuleIn[r])) {
    ...
    RuleBits -= (float) LogFact[Selected];	// allow for reordering of rules
    ...
    ForEach(r, 1, NRules) {
        if(Rule[r].Rhs == FocusClass) {
            if(RuleIn[r]) {
                NewBits = ExceptionBits(SumCover-Right[r]-Wrong[r], FPos-Wrong[r], FNeg+Right[r]) + CodeWeight * (RuleBits - Rule[r].Bits + LogItemNo[Selected]);
                Value[r] = NewBits - BaseBits;
            }
            else {
                NewBits = ExceptionBits(SumCover+Right[r]+Wrong[r], FPos+Wrong[r], FNeg-Right[r]) + CodeWeight * (RuleBits + Rule[r].Bits - LogItemNo[Selected+1]);
...
/*  ---------  */
{
    ...
    ForEach(Pass, 0, MaxClass) {
        ...
        BestC = -1;
        ForEach(c, 0, MaxClass) {
            if(! Included[c] && (BestC < 0 || FalsePos[c] < FalsePos[BestC])) {
                BestC = c;
            }
        }
        Included[BestC] = true;
        ...
        do {
            ...
            ForEach(r, 1, NRules) {
                if(RuleIn[r] && Rule[r].Rhs == BestC && (! BestR || Rule[r].Error < Rule[BestR].Error)) {
                    BestR = r;
                }
            }
            if(BestR) {
    ...
    // INIZIO CODICE NUOVO!!!!
    // delete the rules not included in RuleIndex
    ForEach(r, 1, NRules) {
    	c = 0;
    	ForEach(i, 1, NewNRules) {
            if(r == RuleIndex[i]) {
                c = 1;
                break;
            }
    	...
    	if(c == 0) {
    		ForEach(BestC, 1, Rule[r].Size) {
                    if(Rule[r].Lhs[BestC]->CondTest->NodeType == BrSubset) {
                        ForEach(Pass, 1, Rule[r].Lhs[BestC]->CondTest->Forks) free(Rule[r].Lhs[BestC]->CondTest->Subset[Pass]);
                        free(Rule[r].Lhs[BestC]->CondTest->Subset);
                    }
                    free(Rule[r].Lhs[BestC]->CondTest);
                    free(Rule[r].Lhs[BestC]);
    		}
    		free(Rule[r].Lhs);
    		Rule[r].Lhs = NULL;
    	}
    }
    printf("DENTRO MAKEINDEX, nRULES = %d, NEWRULES = %d\n", NRules, NewNRules);
    // FINE CODICE NUOVO!!!
    NRules = NewNRules;
    free(Included);
}
...
/*  -----------  */
{
    ...
    ForEach(c, 1, MaxClass) {
        if(NoRule[c] > NoRule[DefaultClass] || NoRule[c] == NoRule[DefaultClass] && ClassFreq[c] > ClassFreq[DefaultClass]) {
            DefaultClass = c;
        }
    }
}
...
/*    ------  */
{
    float Rate;
    if(ExpE <= 1E-6) {
        return (float) ((E == 0 ? 0.0 : 1E6));
    }
    else if(ExpE >= N-1E-6) {
        return (float) ((E == N ? 0.0 : 1E6));
    }
    Rate = (float) (ExpE / N);
...
/*    -------------  */
{
    if(Fires > 0.5 * (MaxItem+1)) {
        return Log(MaxItem+1) + Biased(Fires, FP, 0.5 * (FP+FN)) + Biased(MaxItem+1-Fires, FN, (float) FN);
    }
    else {
        return Log(MaxItem+1) + Biased(Fires, FP, (float) FP) + Biased(MaxItem+1-Fires, FN, 0.5 * (FP+FN));
    }
}
...
/*  -------------  */
{
    ...
    ForEach(r, 1, NRules) {
        ...
        // Must encode the number of conditions, but credit the total encoding by the ways conditions can be reordered
