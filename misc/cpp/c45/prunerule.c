...
extern ItemNo * Errors;				// [Condition]
extern ItemNo * Total;				// [Condition]
extern float * Pessimistic;			// [Condition]
extern float * Actual;				// [Condition]
extern float * CondSigLevel;		// [Condition]
extern Boolean * * CondSatisfiedBy;	// [Condition][ItemNo]
extern Boolean * Deleted;

#define Before(n1,n2)  (n1->Tested < n2->Tested || n1->NodeType < n2->NodeType || n1->Tested == n2->Tested && n1->Cut < n2->Cut)
#define IsTarget(case)  (Class(case) == TargetClass ? 1 : 0)

void PruneRule(Condition Cond[], short NCond, ClassNo TargetClass);
Boolean Redundant(short R, Condition Cond[], short NCond);
Boolean IsSubset(Set S1, Set S2, Attribute Att);
...
/*  ---------  */
{
    ...
    do {
		...
		// Find the condition, deleting which would most improve the accuracy of the rule.
	    // Notes: The pessimistic error rate, and not the actual error rate, is currently used.
		...
		ForEach(d, 0, NCond) {
	    	...
	    	Verbosity(1) printf("   %5d%5d  %4.1f%%", Errors[d], Total[d], 100 * Pessimistic[d]);
	    	if(! d) {
				Verbosity(1) printf("\t<base rule>\n");
	    	}
	    	else {
				id++;
				// If significance testing option used, invoke Fisher's exact test here to assess probability
				// that division by d arises from chance.
				if(SIGTEST) {
		    		CondSigLevel[d] = TableProb(Errors[0], Errors[d]-Errors[0], Total[0]-Errors[0], Total[d]-Total[0]-Errors[d]+Errors[0]);
		    		Verbosity(1) printf("  Sig=%.3f", CondSigLevel[d]);
				}
				Verbosity(1) PrintCondition(Cond[d]);
				// Bestd identifies the condition with lowest pessimistic error estimate
				if(! Bestd || Pessimistic[d] <= Pessimistic[Bestd]) {
		    		Bestd = d;
		    		Bestid = id;
				}
				// Alter is set true if we are going to drop a condition (either because we get lower pessimistic est,
		    	// or because one of the conditions fails a significance test)
				if(Pessimistic[d] <= Pessimistic[0] || Actual[d] <= Actual[0] || SIGTEST && CondSigLevel[d] > SIGTHRESH) {
		    		Alter = true;
				}
	    	}
		}
    ...
    ForEach(d, 1, Remaining) {
		dd =  0;
		ForEach(id, d, NCond) {
	    	if(! Deleted[id] && (! dd || Before(Cond[id]->CondTest, Cond[dd]->CondTest))) {
				dd = id;
	    	}
		}
		if(dd != d) {
    ...
    //printf("dentro PruneRule in fondo, chiama NewRule con noCondition = %d\n", Remaining);
    ...
    ForEach(d, 1, NCond) {
		...
		switch(t->NodeType) {
	    	...
	    	case ThreshContin:  // test of continuous attribute
				if(vv == v && (v == 1 ? t->Cut < Rt->Cut : t->Cut > Rt->Cut)) {
		    		return true;
				}
				break;
...
/*               ---------  */
{
    return exp(LogFact[A] + LogFact[B] + LogFact[r] + LogFact[A+B-r] - (LogFact[a] + LogFact[r-a] + LogFact[A-a] + LogFact[B-(r-a)] + LogFact[A+B]));
}
...
/*    ---------  */
{
    double Sum=0.0;
    int A, B, r, a, k, a0;
    // First, rearrange the rows and columns of the table to get it into canonical form
    if(t11 + t12 > t21 + t22) {
		A = t11 + t12;
		B = t21 + t22;
