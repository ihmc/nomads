...
{
    const char *pszMethodName = "RankFactory::getRank";
    if (pMetadata == NULL) {
        checkAndLogMsg (pszMethodName, Logger::L_Warning, "null metadata.\n");
        return RankFactory::getZeroRank (NULL, NULL, bInstrumented);
    }
    String msgId;
    if (pMetadata->getFieldValue (MetadataInterface::MESSAGE_ID, msgId) < 0 || msgId.length() <= 0) {
        checkAndLogMsg (pszMethodName, Logger::L_Warning, "metadata does not contain %s.\n",
                        MetadataInterface::MESSAGE_ID);
        return RankFactory::getZeroRank (pMetadata, NULL, bInstrumented);
    }
