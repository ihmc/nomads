...
namespace MATCHMAKING_POLICIES
{
    Match rankByAreaOfInterest (const BoundingBox &metadataBBox, const BoundingBox &areaOfInterestBBox)
    {
        if (!(areaOfInterestBBox.getArea() > 0.0f)) {
            return Match (Match::NO);
        }
        if (!(metadataBBox.getArea() > 0.0f)) {
            return Match (Match::NO);
        }
        BoundingBox intersection (metadataBBox.getIntersection (areaOfInterestBBox));
        if (!intersection.isValid()) {
            return Match (Match::NO);
        }
        if (!(intersection.getArea() > 0.0f)) {
            return Match (Match::NO);
        }
        float fOverlapRatio = minimum (fabs (intersection.getArea() / metadataBBox.getArea()), 1.0f);
        return Match (Match::YES, fOverlapRatio * 10); // Confidence is within 0 and 10
    }
}

Match MatchMakingPolicies::rankByAreasOfInterest (const BoundingBox &metadataBBox, AreaOfInterestList *pAreasOfInterest)
{
    const char *pszMethodName = "MatchMakingPolicies::rankByAreaOfInterest";
    if (pAreasOfInterest == NULL) {
        return Match (Match::NO);
    }
    ...
    AreaOfInterest *pArea = pAreasOfInterest->getFirst();
    for (; pArea != NULL; pArea = pAreasOfInterest->getNext()) {
        String name (pArea->getName());
        const BoundingBox bb (pArea->getBoundingBox());
        Match m (MATCHMAKING_POLICIES::rankByAreaOfInterest (metadataBBox, bb));
        if ((m._match > match._match) || (m._fMatchConfidence > match._fMatchConfidence)) {
            match = m;
            matchingAreaName = name;
        }
    }
    if (match._match == Match::YES) {
        checkAndLogMsg (pszMethodName, Logger::L_Info, "area of interest %s matched.\n", matchingAreaName.c_str());
    }
    return match;
}
                                                 ...
                                                 bool bConsiderFuturePathSegmentForMatchmacking)
{
    ...
    if ((pPath == NULL) || (iMetadataWayPoint < 0) || (iMetadataWayPoint > pPath->getPathLength())) {
        return Match (Match::NOT_SURE);
    }
    ...
    if (iCurrentWayPoint == -1) {
        ...
        assert ((fRawRank >= 0.0f) && (fRawRank <= 1.0f));
        fRawRank = 1.0f - fRawRank;
        float fRank = scale (fRawRank, 0.0f, 1.0f, MetadataRankerLocalConfiguration::MIN_RANK,
...
{
    const char *pszMethodName = "MatchMakingPolicies::rankByExpirationTime";
    if ((ui64ExpirationTimeStamp == 0) || (ui64ExpirationTimeStamp == 1)) {
        return Match (Match::NOT_SURE);
    }
    if (ui64SourceTimeStamp == (uint64) MetadataInterface::SOURCE_TIME_STAMP_UNSET) {
        checkAndLogMsg (pszMethodName, Logger::L_MildError, "The source time stamp in the "
                        "given MetaData is an unknown value. Could not rank the metadata.\n");
        return Match (Match::NOT_SURE);
    }
    const uint64 ui64CurrTime = getTimeInMilliseconds();
    if (ui64CurrTime < ui64SourceTimeStamp) {
        // Time skew!  Can't make a decision...
        checkAndLogMsg (pszMethodName, Logger::L_MildError, "The source time stamp in the "
                        "given MetaData is in the future. Could not rank the metadata.\n");
        return Match (Match::NOT_SURE);
    }
    const uint64 ui64AbsoluteExpTimeStamp = ui64SourceTimeStamp + ui64ExpirationTimeStamp;
    if (ui64AbsoluteExpTimeStamp > ui64CurrTime) {
        float fRank = (ui64AbsoluteExpTimeStamp - ui64CurrTime) * MetadataRankerLocalConfiguration::MAX_RANK / ui64ExpirationTimeStamp;
        if (fRank > MetadataRankerLocalConfiguration::MIN_RANK) {
            return Match(Match::YES, fRank);
        }
    }
...
{
    const char *pszMethodName = "MatchMakingPolicies::rankByTarget";
    ...
    const String role (pNodeContext->getRole());
    const String teamId (pNodeContext->getTeamId());
    const String missiondId (pNodeContext->getMissionId());
    rankByTarget (role, pMetaDataTargetRole, fAchievedMatch, fMaximumAchievableMatch);
    rankByTarget (teamId, pMetaDataTargetTeam, fAchievedMatch, fMaximumAchievableMatch);
    rankByTarget (missiondId, pMetaDataTargetMission, fAchievedMatch, fMaximumAchievableMatch);
    ...
    if (fMaximumAchievableMatch > 0.0f) {
        ...
        if (bError) {
            ...
            checkAndLogMsg (pszMethodName, Logger::L_SevereError, "scale returned an error for "
                            "arguments %f, 0.0, %f, %f, %f", fAchievedMatch, fMaximumAchievableMatch,
                            MetadataRankerLocalConfiguration::MIN_RANK, MetadataRankerLocalConfiguration::MAX_RANK);
            return Match (Match::NOT_SURE);
        }
