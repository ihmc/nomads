...
public class BMPFragmenter {
    ...
    public static BufferedImage fragmentBMP(BufferedImage pSourceImage, int ui8DesiredChunkId, int ui8TotalNoOfChunks) {
        ...
        // create the resulting image - with or without alpha depending on whether the original image has alpha
        BufferedImage pChunkedImage = null;
        if (pSourceImage.getColorModel().getNumComponents() == 3) {
            // no alpha/transparency
            pChunkedImage = new BufferedImage((int) ui32NewWidth, (int) ui32NewHeight, BufferedImage.TYPE_3BYTE_BGR);
        }
        else {
            // has alpha/transparency
            pChunkedImage = new BufferedImage((int) ui32NewWidth, (int) ui32NewHeight, BufferedImage.TYPE_4BYTE_ABGR);
        }
