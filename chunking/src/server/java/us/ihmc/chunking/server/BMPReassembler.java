 ...
 * Based on the idea that chunks are created by taking different 'stripes' of pixels from the original image:
 * 
 * original:	0123
 * 				4567
 * 
 * chunk 1: 	02		chunk 2:	13
 * 				46					57
 * 
...
public class BMPReassembler {
	...
	// this array will contain true at the index of each chunk id that has already been woven back into the result
	 ...
	 * this method 'fills in the blanks' of missing chunks by averaging the RGB values of chunks that have been incorporated.
	 * 
	...
	/**
	 * Create a blank reassembled result image based on the size and image type (e.g. alpha/no alpha) of the given smaller chunk image.
	 * @param pChunk
	 * @return
	 */
	private BufferedImage initBlankReassembledImage(BufferedImage pChunk) {
		BufferedImage blankReassembleImage = null;
		if (pChunk.getColorModel().getNumComponents() == 3) {
			blankReassembleImage = new BufferedImage (pChunk.getWidth() * _ui8XIncr, pChunk.getHeight() * _ui8YIncr, BufferedImage.TYPE_3BYTE_BGR);
	    }
	    else {
	    	blankReassembleImage = new BufferedImage (pChunk.getWidth() * _ui8XIncr, pChunk.getHeight() * _ui8YIncr, BufferedImage.TYPE_4BYTE_ABGR);
	    }
