...
#define FIX3(x) ((INT32) ((x) * CONST_SCALE + 1))
...
/*
 * This function perform IDCT on ScBlock with DC != 0 and all zero
 * AC values.
 */
void
DctToByteSparse(data, byte, w)
    DCTBLOCK data;
    unsigned char *byte;
    int w;
{
    ...
    memset(byte, val, 8);
    memset(byte + w, val, 8);
    memset(byte + 2 * w, val, 8);
    memset(byte + 3 * w, val, 8);
    memset(byte + 4 * w, val, 8);
    memset(byte + 5 * w, val, 8);
    memset(byte + 6 * w, val, 8);
    memset(byte + 7 * w, val, 8);
}
            ...
            if (d4) {
                if (d2) {
                    if (d0) {
                        ...
                        tmp3 = MULTIPLY(d2, (FIX3(1.306562965)));
 ...
 */
void
DctToByte(data, byte, w)
    DCTBLOCK data;
    unsigned char *byte;
    int w;                      /* parentWidth of byte */
{
    INT32 tmp0, tmp1, tmp2, tmp3;
    INT32 tmp10, tmp11, tmp12, tmp13;
    INT32 z1, z2, z3, z4, z5;
    ...
    for (rowctr = DCTSIZE - 1; rowctr >= 0; rowctr--) {
        ...
        curr = byte;
        *curr = CROP((DCTELEM) DESCALE(tmp10 + tmp3, CONST_BITS + PASS1_BITS + 3));
        curr += w;
        *curr = CROP((DCTELEM) DESCALE(tmp11 + tmp2, CONST_BITS + PASS1_BITS + 3));
        curr += w;
        *curr = CROP((DCTELEM) DESCALE(tmp12 + tmp1, CONST_BITS + PASS1_BITS + 3));
        curr += w;
        *curr = CROP((DCTELEM) DESCALE(tmp13 + tmp0, CONST_BITS + PASS1_BITS + 3));
        curr += w;
        *curr = CROP((DCTELEM) DESCALE(tmp13 - tmp0, CONST_BITS + PASS1_BITS + 3));
        curr += w;
        *curr = CROP((DCTELEM) DESCALE(tmp12 - tmp1, CONST_BITS + PASS1_BITS + 3));
        ...
        *curr = CROP((DCTELEM) DESCALE(tmp10 - tmp3, CONST_BITS + PASS1_BITS + 3));
        ...
        byte++;                 /* advance pointer to next column */
        dataptr++;
    }
}
