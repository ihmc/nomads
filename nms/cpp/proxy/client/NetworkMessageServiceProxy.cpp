...
int NetworkMessageServiceProxy::changeEncryptionKey (unsigned char *pchKey, uint32 ui32Len)
{
    synchronized (&_stubMutex);
    if (_pCommHelper == NULL) {
        return false;
    }
    SimpleCommHelper2::Error error = sendLineAndLog (NetworkMessageServiceUnmarshaller::CHANGE_KEY_METHOD, _pCommHelper);
    if (error != SimpleCommHelper2::None) {
        return false;
    }
    Writer *pWriter = _pCommHelper->getWriterRef();
    ...
    if (pWriter->writeBytes (pchKey, ui32Len) < 0) {
        error = SimpleCommHelper2::CommError;
        return false;
    }
    int8 rc = 1;
    Reader *pReader = _pCommHelper->getReaderRef();
    if (pReader->read8 (&rc) < 0) {
        error = SimpleCommHelper2::CommError;
        return false;
    }
    return rc;
...
int NetworkMessageServiceProxy::messageArrived (const char *pszIncomingInterface, uint32 ui32SourceIPAddress, uint8 ui8MsgType,
                                                uint16 ui16MsgId, uint8 ui8HopCount, uint8 ui8TTL, bool bIsUnicast,
                                                const void *pMsgMetaData, uint16 ui16MsgMetaDataLen,
                                                ...
                                                uint64 ui64GroupMsgCount, uint64 ui64UnicastMsgCount)
{
    ...
    _pListener->messageArrived (pszIncomingInterface, ui32SourceIPAddress, ui8MsgType, ui16MsgId,
                                ui8HopCount, ui8TTL, bIsUnicast, pMsgMetaData, ui16MsgMetaDataLen,
